...
namespace Crescendo.API.Editor {
    ...
    public static class CrescendoEditor
    {
        ...
        static CrescendoEditor() {
            GenerateAutogeneratedFiles();   
        }

        static void GenerateAutogeneratedFiles() {
            foreach (var guid in AssetDatabase.FindAssets("t:CharacterEditorData")) {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                var data = AssetDatabase.LoadAssetAtPath<CharacterEditorData>(path);
                if (!AssetUtil.IsValidFolder(data.GeneratedFolder)) {
                    Debug.Log(data.InternalName + " does not have it's AutoGenerated folder. Generating...");
                    data.Generate();
                }
            }
