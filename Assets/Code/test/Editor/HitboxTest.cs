...
public class HitboxTest : HitboxTestBase {
  ...
  public void GetKnockbackScale_cannot_be_negative(float baseKnockback, float scaling, float damage) {
    var scale = CreateHitbox().WithKnockback(new LinearScaledValue {
        BaseValue = baseKnockback,
        ScalingFactor = scaling
    }).Build().GetKnockbackScale(damage);
  ...
  public void GetKnockbackScale_has_unscaling_base_knockback(float baseKnockback, float scaling, float damage, float knockback) {
    var scale = CreateHitbox().WithKnockback(new LinearScaledValue {
        BaseValue = baseKnockback,
        ScalingFactor = scaling
    }).Build().Info.GetKnockbackScale(damage);
    var globalScaling = Config.Get<PhysicsConfig>().GlobalKnockbackScaling;
    Assert.AreEqual(scale, knockback * globalScaling, 0.001);
  }
  ...
  public void GetHitstun_cannot_be_negative(int baseHitstun, float scaling, float damage) {
    var hitstun = CreateHitbox().WithHitstun(new LinearScaledValue {
        BaseValue = baseHitstun,
        ScalingFactor = scaling
    }).Build().Info.GetHitstun(damage);
  ...
  public void GetKnockbackDirection_corresponds_to_knockback_angle(float angle, float x, float y) {
    var knockbackDirection = CreateHitbox().WithMirrorDirection(false).WithKnockbackAngle(angle).Build().Info.GetKnockbackDirection(false);
    Assert.AreEqual(x, knockbackDirection.x, 0.000001);
    Assert.AreEqual(y, knockbackDirection.y, 0.000001);
    knockbackDirection = CreateHitbox().WithMirrorDirection(false).WithKnockbackAngle(angle).Build().Info.GetKnockbackDirection(true);
    Assert.AreEqual(x, knockbackDirection.x, 0.000001);
    Assert.AreEqual(y, knockbackDirection.y, 0.000001);
  }
  ...
  public void GetKnockbackDirection_mirrors_direction(float angle, float x, float y) {
    var knockbackDirection = CreateHitbox().WithMirrorDirection(true).WithKnockbackAngle(angle).Build().Info.GetKnockbackDirection(false);
    Assert.AreEqual(-x, knockbackDirection.x, 0.000001);
    Assert.AreEqual(y, knockbackDirection.y, 0.000001);
    knockbackDirection = CreateHitbox().WithMirrorDirection(true).WithKnockbackAngle(angle).Build().Info.GetKnockbackDirection(true);
    Assert.AreEqual(x, knockbackDirection.x, 0.000001);
    Assert.AreEqual(y, knockbackDirection.y, 0.000001);
  }
    ...
    public void GetKnockback_scales_with_damage(float damage, float angle, float x, float y) {
        var knockback = CreateHitbox().WithKnockback(new LinearScaledValue {
            BaseValue = 0,
            ScalingFactor = 1
        }) .Build().Info.GetKnocback(damage, true);
	...
	public void CollisionCheck_returns_zero_if_no_nearby_colliders() {
        ...
        Assert.AreEqual(0, hitbox.GetCollidedHurtboxes(new Hurtbox[10]));
	}
    ...
    CreateObject<SphereCollider>().transform.position = 0.5f * Vector3.right;
    Assert.AreEqual(1, hitbox.GetCollidedHurtboxes(new Hurtbox[10]));
	}
    ...
    Assert.AreEqual(2, hitbox.GetCollidedHurtboxes(hurtboxes));
    ...
    CreateObject<SphereCollider>().transform.position = 0.5f * Vector3.right;
    var hurtboxes = new Hurtbox[10];
    var count = hitbox.GetCollidedHurtboxes(hurtboxes);
