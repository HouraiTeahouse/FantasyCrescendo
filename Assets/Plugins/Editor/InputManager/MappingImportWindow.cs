...
namespace TeamUtility.Editor.IO.InputManager {
    ...
    public class MappingImportWindow : EditorWindow {
        ...
        [SerializeField]
        private AdvancedInputEditor _configurator;
        ...
        private void OnGUI() {
            var importButtonHeight = 24.0f;
            var toolbarPosition = new Rect(0.0f, 0.0f, position.width, _toolbarHeight);
            var mappingListPosition = new Rect(0.0f,
                                               _toolbarHeight,
                                               position.width,
                                               position.height - (_toolbarHeight + importButtonHeight + 10.0f));
            var importButtonPosition = new Rect(position.width/2 - 100.0f,
                                                position.height - importButtonHeight - 5.0f,
                                                200.0f,
                                                importButtonHeight);
        ...
        private void UpdateSearchResults() {
            ...
            for (var i = 0; i < _mappings.Count; i++) {
                if (_mappings[i].Name.IndexOf(_searchString, StringComparison.InvariantCultureIgnoreCase) >= 0)
        ...
        private void ImportSelectedMapping() {
            if (_configurator == null) {
                EditorUtility.DisplayDialog("Error",
                                            "Unable to import joystick mapping. Did you close the advanced input editor?",
                                            "Close");
                return;
            }

            var inputConfig = new InputConfiguration(_mappings[_selection].Name.Replace(' ', '_'));
            foreach (AxisMapping am in _mappings[_selection]) {
                if (am.ScanType == MappingWizard.ScanType.Button) {
                    var axisConfig = new AxisConfiguration(am.Name);
        ...
        public static void Open(AdvancedInputEditor configurator) {
            var window = GetWindow<MappingImportWindow>("Mapping Importer");
