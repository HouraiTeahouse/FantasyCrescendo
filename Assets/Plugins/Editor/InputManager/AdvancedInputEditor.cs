...
namespace TeamUtility.Editor {
    ...
    public sealed class AdvancedInputEditor : EditorWindow {
        ...
        private void ExportInputConfigurations() {
            ...
            var inputSaver = new InputSaverXML(file);
        ...
        private void ImportInputConfigurations() {
            ...
            bool replace = EditorUtility.DisplayDialog("Replace or Append",
                                                       "Do you want to replace the current input configrations?",
                                                       "Replace",
                                                       "Append");
            if (replace) {
                var inputLoader = new InputLoaderXML(file);
                ...
                if (configurations != null && configurations.Count > 0) {
                    foreach (InputConfiguration config in configurations)
        ...
        private void LoadInputConfigurationsFromResource(string resourcePath) {
            if (_inputManager.inputConfigurations.Count > 0) {
                bool cont = EditorUtility.DisplayDialog("Warning",
                                                        "This operation will replace the current input configrations!\nDo you want to continue?",
                                                        "Yes",
                                                        "No");
                if (!cont)
                    return;
            ...
            if (textAsset != null) {
                using (var reader = new System.IO.StringReader(textAsset.text)) {
                    var inputLoader = new InputLoaderXML(reader);
                    inputLoader.Load(out _inputManager.inputConfigurations, out _inputManager.defaultConfiguration);
                    _selectionPath.Clear();
                }
            } else {
                EditorUtility.DisplayDialog("Error",
                                            "Failed to load input configurations. The resource file might have been deleted or renamed.",
                                            "OK");
            }
        }
        ...
        private bool _tryedToFindInputManagerInScene;
        ...
        private void CreateFileMenu(Rect position) {
            ...
            fileMenu.AddItem(new GUIContent("Overwrite Input Settings"),
                             false,
                             HandleFileMenuOption,
                             FileMenuOptions.OverriteInputSettings);
            fileMenu.AddItem(new GUIContent("Default Input Configuration"),
                             false,
                             HandleFileMenuOption,
                             FileMenuOptions.CreateDefaultInputConfig);
            if (EditorToolbox.HasInputAdapterAddon()) {
                fileMenu.AddItem(new GUIContent("Configure For Input Adapter"),
                                 false,
                                 HandleFileMenuOption,
                                 FileMenuOptions.ConfigureForInputAdapter);
            }

            fileMenu.AddSeparator("");
            if (_inputManager.inputConfigurations.Count > 0) {
                fileMenu.AddItem(new GUIContent("Create Snapshot"),
                                 false,
                                 HandleFileMenuOption,
                                 FileMenuOptions.CreateSnapshot);
            } else
                fileMenu.AddDisabledItem(new GUIContent("Create Snapshot"));

            if (EditorToolbox.CanLoadSnapshot()) {
                fileMenu.AddItem(new GUIContent("Load Snapshot"),
                                 false,
                                 HandleFileMenuOption,
                                 FileMenuOptions.LoadSnapshot);
            } else
            ...
            if (EditorToolbox.HasJoystickMappingAddon()) {
                fileMenu.AddItem(new GUIContent("Import Joystick Mapping"),
                                 false,
                                 HandleFileMenuOption,
                                 FileMenuOptions.ImportJoystickMapping);
            }
        ...
        private void HandleFileMenuOption(object arg) {
            var option = (FileMenuOptions) arg;
            switch (option) {
        ...
        private void CreateEditMenu(Rect position) {
            ...
            editMenu.AddItem(new GUIContent("New Configuration"),
                             false,
                             HandleEditMenuOption,
                             EditMenuOptions.NewInputConfiguration);
            if (_selectionPath.Count >= 1) {
                editMenu.AddItem(new GUIContent("New Axis"),
                                 false,
                                 HandleEditMenuOption,
                                 EditMenuOptions.NewAxisConfiguration);
            } else
                editMenu.AddDisabledItem(new GUIContent("New Axis"));
            editMenu.AddSeparator("");

            if (_selectionPath.Count > 0) {
                editMenu.AddItem(new GUIContent("Duplicate          Shift+D"),
                                 false,
                                 HandleEditMenuOption,
                                 EditMenuOptions.Duplicate);
            } else
                editMenu.AddDisabledItem(new GUIContent("Duplicate          Shift+D"));

            if (_selectionPath.Count > 0) {
                editMenu.AddItem(new GUIContent("Delete                Del"),
                                 false,
                                 HandleEditMenuOption,
            ...
            editMenu.AddItem(new GUIContent("Ignore Timescale"),
                             _inputManager.ignoreTimescale,
                             HandleEditMenuOption,
                             EditMenuOptions.IgnoreTimescale);
            editMenu.AddItem(new GUIContent("Dont Destroy On Load"),
                             _inputManager.dontDestroyOnLoad,
                             HandleEditMenuOption,
                             EditMenuOptions.DontDestroyOnLoad);
            editMenu.DropDown(position);
        }

        private void HandleEditMenuOption(object arg) {
            var option = (EditMenuOptions) arg;
            switch (option) {
        ...
        private void UpdateSearchResults() {
            ...
            for (var i = 0; i < _inputManager.inputConfigurations.Count; i++) {
                IEnumerable<int> axes = from a in _inputManager.inputConfigurations[i].axes
                                        where
                                            (a.name.IndexOf(_searchString, StringComparison.InvariantCultureIgnoreCase) >=
                                             0)
        ...
        private void DisplayMissingInputManagerWarning() {
            var warningRect = new Rect(0.0f, 20.0f, position.width, 40.0f);
            var buttonRect = new Rect(position.width/2 - 100.0f, warningRect.yMax, 200.0f, 25.0f);
        ...
        private void DisplayMainToolbar() {
            var screenRect = new Rect(0.0f, 0.0f, position.width, _toolbarHeight);
            var fileMenuRect = new Rect(0.0f, 0.0f, _menuWidth, screenRect.height);
            var editMenuRect = new Rect(fileMenuRect.xMax, 0.0f, _menuWidth, screenRect.height);
            var paddingLabelRect = new Rect(editMenuRect.xMax, 0.0f, screenRect.width - _menuWidth*2, screenRect.height);
            var searchFieldRect = new Rect(screenRect.width - (_menuWidth*1.5f + 5.0f),
                                           2.0f,
                                           _menuWidth*1.5f,
        ...
        private void UpdateHierarchyPanelWidth() {
            ...
            var cursorRect = new Rect(_hierarchyPanelWidth - cursorRectWidth/2,
                                      _toolbarHeight,
                                      cursorRectWidth,
                                      position.height - _toolbarHeight);
            var resizeRect = new Rect(_hierarchyPanelWidth - _minCursorRectWidth/2,
                                      0.0f,
                                      _minCursorRectWidth,
                                      position.height);
        ...
        private void DisplaySearchResults() {
            var screenRect = new Rect(0.0f,
                                      _toolbarHeight - 5.0f,
                                      _hierarchyPanelWidth,
                                      position.height - _toolbarHeight + 10.0f);
            GUI.Box(screenRect, "");

            if (_searchResults.Count > 0) {
                var scrollView = new Rect(screenRect.x,
                                          screenRect.y + 5.0f,
                                          screenRect.width,
                                          position.height - screenRect.y);
        ...
        private void DisplaySearchResult(Rect screenRect, SearchResult result) {
            DisplayHierarchyInputConfigItem(screenRect,
            ...
            if (_inputManager.inputConfigurations[result.configuration].isExpanded) {
                for (var i = 0; i < result.axes.Count; i++) {
                    DisplayHierarchiAxisConfigItem(screenRect,
                                                   result.configuration,
                                                   result.axes[i],
                                                   _inputManager.inputConfigurations[result.configuration].axes[
                                                                                                                result
                                                                                                                    .axes
                                                                                                                    [i]]
                                                       .name);
        ...
        private void DisplayHierarchyPanel() {
            ...
            for (var i = 0; i < _inputManager.inputConfigurations.Count; i++) {
                ...
                if (_inputManager.inputConfigurations[i].isExpanded) {
                    for (var j = 0; j < _inputManager.inputConfigurations[i].axes.Count; j++) {
                        DisplayHierarchiAxisConfigItem(screenRect,
                                                       i,
                                                       j,
                                                       _inputManager.inputConfigurations[i].axes[j].name);
                    }
                }
            }
            GUILayout.Space(5.0f);
        ...
        private void DisplayHierarchyInputConfigItem(Rect screenRect, int index, string name) {
            Rect configPos = GUILayoutUtility.GetRect(new GUIContent(name),
                                                      EditorStyles.foldout,
                                                      GUILayout.Height(_hierarchyItemHeight));
            if (Event.current.type == EventType.MouseDown && Event.current.button == 0) {
            ...
            if (_selectionPath.Count == 1 && _selectionPath[0] == index) {
                ...
                _inputManager.inputConfigurations[index].isExpanded = EditorGUI.Foldout(configPos,
                                                                                        _inputManager
                                                                                            .inputConfigurations[index]
                                                                                            .isExpanded,
                                                                                        name,
                                                                                        _whiteFoldout);
            } else {
        ...
        private void DisplayMainPanel() {
            var screenRect = new Rect(_hierarchyPanelWidth + 5.0f,
                                      _toolbarHeight + 5,
        ...
        private void DisplayAxisConfigurationFields(InputConfiguration inputConfigx,
            ...
            EditorToolbox.KeyCodeField(ref _keyString,
                                       ...
                                       "Alt Positive",
                                       "editor_alt_positive_key",
                                       axisConfig.altPositive);
            ...
            EditorToolbox.KeyCodeField(ref _keyString,
                                       ...
                                       "Alt Negative",
                                       "editor_alt_negative_key",
                                       axisConfig.altNegative);
        ...
        public static void OpenWindow() {
            if (!IsOpen) {
                if (FindObjectOfType(typeof (InputManager)) == null) {
                    var gameObject = new GameObject("InputManager");
                    gameObject.AddComponent<InputManager>();
                }
                GetWindow<AdvancedInputEditor>("Input");
            }
        }

        public static void OpenWindow(InputManager target) {
            if (!IsOpen) {
                var window = GetWindow<AdvancedInputEditor>("Input");
