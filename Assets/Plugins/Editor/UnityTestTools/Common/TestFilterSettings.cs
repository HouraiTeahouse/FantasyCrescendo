...
namespace UnityTest {
    ...
    public class TestFilterSettings {
        ...
        public void UpdateCounters(IEnumerable<ITestResult> results) {
            ...
            _failedBtn = new GUIContent((summary.Errors + summary.Failures + summary.Inconclusive).ToString(),
                                        Icons.FailImg,
                                        "Show tests that failed");
            _ignoredBtn = new GUIContent((summary.Ignored + summary.NotRunnable).ToString(),
                                         Icons.IgnoreImg,
                                         "Show tests that are ignored");
            _notRunBtn = new GUIContent((summary.TestsNotRun - summary.Ignored - summary.NotRunnable).ToString(),
                                        Icons.UnknownImg,
        ...
        public void OnGUI() {
            ...
            FilterByName = GUILayout.TextField(FilterByName,
                                               "ToolbarSeachTextField",
                                               GUILayout.MinWidth(100),
                                               GUILayout.MaxWidth(250),
                                               GUILayout.ExpandWidth(true));
            if (GUILayout.Button(GUIContent.none,
                                 string.IsNullOrEmpty(FilterByName)
            ...
            if (AvailableCategories != null && AvailableCategories.Length > 0) {
                FilterByCategory = EditorGUILayout.MaskField(FilterByCategory,
                                                             AvailableCategories,
