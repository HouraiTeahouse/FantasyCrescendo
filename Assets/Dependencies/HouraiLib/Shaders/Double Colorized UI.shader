...
#pragma multi_compile __ UNITY_UI_ALPHACLIP
	...
	{
		half4 targetColor = IN.color;
		half4 texColor = (tex2D(_MainTex, IN.texcoord) + _TextureSampleAdd);
		fixed greyScale = (texColor.r + texColor.g + texColor.b) / 3;
		half4 color;
		if(greyScale > 0.5)
			color = lerp(targetColor, _Top, 2 * (greyScale - 0.5)) * (1, 1, 1, texColor.a * targetColor.a);
		else
			color = lerp(_Bottom, targetColor, 2 * greyScale) * (1, 1, 1, texColor.a * targetColor.a); 
