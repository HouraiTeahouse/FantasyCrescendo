...
namespace UniRx.Operators {
    ...
    internal class TakeObservable<T> : OperatorObservableBase<T> {
        ...
        public TakeObservable(IObservable<T> source, int count) : base(source.IsRequiredSubscribeOnCurrentThread()) {
            this.source = source;
            this.count = count;
        }
        ...
        public IObservable<T> Combine(int count) {
            ...
            // use minimum one
            return this.count <= count ? this : new TakeObservable<T>(source, count);
        }
        ...
        public IObservable<T> Combine(TimeSpan duration) {
            ...
            // use minimum one
            return this.duration <= duration ? this : new TakeObservable<T>(source, duration, scheduler);
        }
