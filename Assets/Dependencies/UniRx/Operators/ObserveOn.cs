    ...
    internal class ObserveOnObservable<T> : OperatorObservableBase<T> {
        ...
        class ObserveOn : OperatorObserverBase<T, T> {
            ...
            public IDisposable Run() {
                ...
                return StableCompositeDisposable.Create(sourceDisposable,
                    Disposable.Create(() => {
                        lock (scheduleDisposables) {
                            isDisposed = true;

                            foreach (IDisposable item in scheduleDisposables) {
                                item.Dispose();
                            }
        ...
        class ObserveOn_ : OperatorObserverBase<T, T> {
            ...
            public ObserveOn_(ObserveOnObservable<T> parent,
            ...
            void OnNext_(T value) { observer.OnNext(value); }
            ...
            public override void OnNext(T value) { scheduler.ScheduleQueueing(isDisposed, value, onNext); }

            public override void OnError(Exception error) { scheduler.ScheduleQueueing(isDisposed, error, OnError_); }

            public override void OnCompleted() { scheduler.ScheduleQueueing(isDisposed, Unit.Default, OnCompleted_); }
