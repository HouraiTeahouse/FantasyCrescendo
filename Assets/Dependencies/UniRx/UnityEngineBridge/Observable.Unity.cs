...
namespace UniRx {
    ...
    public class ObservableYieldInstruction<T> : IEnumerator<T>, ICustomYieldInstructionErrorHandler {
        ...
        class ToYieldInstruction : IObserver<T> {
            ...
            public ToYieldInstruction(ObservableYieldInstruction<T> parent) { this.parent = parent; }

            public void OnNext(T value) { parent.current = value; }
        ...
        /// <summary> HasResult || IsCanceled || HasError </summary>
        public bool IsDone {
        ...
        void ICustomYieldInstructionErrorHandler.ForceDisableRethrowOnError() { reThrowOnError = false; }

        void ICustomYieldInstructionErrorHandler.ForceEnableRethrowOnError() { reThrowOnError = true; }
...
#endif
    {
        ...
        /// <summary> From has no callback coroutine to IObservable. If publishEveryYield = true then publish OnNext every yield
        /// return else return once on enumeration completed. </summary>
        ...
        /// <summary> MicroCoroutine is lightweight, fast coroutine dispatcher. IEnumerator supports only yield return null. If
        /// publishEveryYield = true then publish OnNext every yield return else return once on enumeration completed. </summary>
        public static IObservable<Unit> FromMicroCoroutine(Func<IEnumerator> coroutine,
                                                           bool publishEveryYield = false,
                                                           FrameCountType frameCountType = FrameCountType.Update) {
            return
                FromMicroCoroutine<Unit>(
                    (observer, cancellationToken) =>
                        WrapEnumerator(coroutine(), observer, cancellationToken, publishEveryYield),
                    frameCountType);
        }
        ...
        static IEnumerator WrapEnumerator(IEnumerator enumerator,
        ...
        /// <summary> Convert coroutine to typed IObservable. If nullAsNextUpdate = true then yield return null when
        /// Enumerator.Current and no null publish observer.OnNext. </summary>
        ...
        public static IObservable<T> FromCoroutineValue<T>(Func<CancellationToken, IEnumerator> coroutine,
            ...
            return
                FromCoroutine<T>(
                    (observer, cancellationToken) =>
                        WrapEnumeratorYieldValue<T>(coroutine(cancellationToken),
                            observer,
                            cancellationToken,
                            nullAsNextUpdate));
        }

        static IEnumerator WrapEnumeratorYieldValue<T>(IEnumerator enumerator,
        ...
        public static IObservable<T> FromCoroutine<T>(Func<IObserver<T>, IEnumerator> coroutine) {
            return
                FromCoroutine<T>(
                    (observer, cancellationToken) => WrapToCancellableEnumerator(coroutine(observer), cancellationToken));
        }

        /// <summary> MicroCoroutine is lightweight, fast coroutine dispatcher. IEnumerator supports only yield return null. </summary>
        public static IObservable<T> FromMicroCoroutine<T>(Func<IObserver<T>, IEnumerator> coroutine,
                                                           FrameCountType frameCountType = FrameCountType.Update) {
            return
                FromMicroCoroutine<T>(
                    (observer, cancellationToken) => WrapToCancellableEnumerator(coroutine(observer), cancellationToken),
                    frameCountType);
        }
        ...
        public static IObservable<Unit> SelectMany<T>(this IObservable<T> source,
        ...
        /// selector(x), true). This is workaround of Unity compiler's bug. </summary>
        ...
        /// <summary> EveryUpdate calls coroutine's yield return null timing. It is after all Update and before LateUpdate. </summary>
        ...
        /// <summary> EveryGameObjectUpdate calls from MainThreadDispatcher's Update. </summary>
        public static IObservable<long> EveryGameObjectUpdate() {
            return MainThreadDispatcher.UpdateAsObservable().Scan(-1L, (x, y) => x + 1);
        }

        /// <summary> EveryLateUpdate calls from MainThreadDispatcher's OnLateUpdate. </summary>
        ...
        /// <summary> [Obsolete]Same as EveryUpdate. </summary>
        [Obsolete]
        public static IObservable<long> EveryAfterUpdate() {
            return
                FromCoroutine<long>(
                    (observer, cancellationToken) => new EveryAfterUpdateInvoker(observer, cancellationToken));
        }
        ...
        public static IObservable<long> TimerFrame(int dueTimeFrameCount,
                                                   FrameCountType frameCountType = FrameCountType.Update) {
            return
                FromMicroCoroutine<long>(
                    (observer, cancellation) => TimerFrameCore(observer, dueTimeFrameCount, cancellation),
                    frameCountType);
        }
            ...
            return
                FromMicroCoroutine<long>(
                    (observer, cancellation) =>
                        TimerFrameCore(observer, dueTimeFrameCount, periodFrameCount, cancellation),
                    frameCountType);
        }
        ...
        static IEnumerator TimerFrameCore(IObserver<long> observer,
        ...
        public static IObservable<T> DelayFrame<T>(this IObservable<T> source,
                                                   int frameCount,
                                                   FrameCountType frameCountType = FrameCountType.Update) {
            if (frameCount < 0)
        ...
        public static IObservable<T> SampleFrame<T>(this IObservable<T> source,
                                                    int frameCount,
                                                    FrameCountType frameCountType = FrameCountType.Update) {
            if (frameCount < 0)
                throw new ArgumentOutOfRangeException("frameCount");
        ...
        public static IObservable<TSource> ThrottleFrame<TSource>(this IObservable<TSource> source,
                                                                  int frameCount,
                                                                  FrameCountType frameCountType = FrameCountType.Update) {
            if (frameCount < 0)
                throw new ArgumentOutOfRangeException("frameCount");
        ...
        public static IObservable<TSource> ThrottleFirstFrame<TSource>(this IObservable<TSource> source,
                                                                       int frameCount,
        ...
        public static IObservable<T> TimeoutFrame<T>(this IObservable<T> source,
                                                     int frameCount,
                                                     FrameCountType frameCountType = FrameCountType.Update) {
            if (frameCount < 0)
                throw new ArgumentOutOfRangeException("frameCount");
        ...
        public static IObservable<T> DelayFrameSubscription<T>(this IObservable<T> source,
                                                               int frameCount,
                                                               FrameCountType frameCountType = FrameCountType.Update) {
            if (frameCount < 0)
                throw new ArgumentOutOfRangeException("frameCount");
        ...
        /// <summary> Convert to yieldable IEnumerator. e.g. yield return source.ToYieldInstruction();. If needs last result, you
        /// can take ObservableYieldInstruction.HasResult/Result property. This overload throws exception if received OnError
        /// events(same as coroutine). </summary>
        ...
        /// <summary> Convert to yieldable IEnumerator. e.g. yield return source.ToYieldInstruction();. If needs last result, you
        /// can take ObservableYieldInstruction.HasResult/Result property. If throwOnError = false, you can take
        /// ObservableYieldInstruction.HasError/Error property. </summary>
...
#pragma warning restore 612 // Type or member is obsolete
        ...
        public static IObservable<T> RepeatUntilDestroy<T>(this IObservable<T> source, Component target) {
            return RepeatUntilCore(RepeatInfinite(source),
        ...
        static IObservable<T> RepeatUntilCore<T>(this IEnumerable<IObservable<T>> sources,
        ...
        /// <summary> Buffer elements in during target frame counts. Default raise same frame of end(frameCount = 0, frameCountType
        /// = EndOfFrame). </summary>
        ...
        /// <summary> Buffer elements in during target frame counts. </summary>
        public static IObservable<IList<T>> BatchFrame<T>(this IObservable<T> source,
                                                          int frameCount,
                                                          FrameCountType frameCountType) {
            if (frameCount < 0)
        ...
        /// <summary> Wait command in during target frame counts. Default raise same frame of end(frameCount = 0, frameCountType =
        /// EndOfFrame). </summary>
        ...
        /// <summary> Wait command in during target frame counts. </summary>
        public static IObservable<Unit> BatchFrame(this IObservable<Unit> source,
                                                   int frameCount,
                                                   FrameCountType frameCountType) {
            if (frameCount < 0)
