...
namespace UniRx {
...
#endif
    {
        ...
        public ReactiveDictionary() { inner = new Dictionary<TKey, TValue>(); }

        public ReactiveDictionary(IEqualityComparer<TKey> comparer) { inner = new Dictionary<TKey, TValue>(comparer); }

        public ReactiveDictionary(Dictionary<TKey, TValue> innerDictionary) { inner = innerDictionary; }
        ...
        public Dictionary<TKey, TValue>.Enumerator GetEnumerator() { return inner.GetEnumerator(); }
        ...
        bool IDictionary.Contains(object key) { return ((IDictionary) inner).Contains(key); }

        void ICollection.CopyTo(Array array, int index) { ((IDictionary) inner).CopyTo(array, index); }
        ...
        public void OnDeserialization(object sender) { ((IDeserializationCallback) inner).OnDeserialization(sender); }
        ...
        IDictionaryEnumerator IDictionary.GetEnumerator() { return ((IDictionary) inner).GetEnumerator(); }
